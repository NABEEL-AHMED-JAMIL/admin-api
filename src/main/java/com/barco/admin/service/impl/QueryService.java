...
public class QueryService {
    ...
    // Query
    public static String FETCH_ALL_QUERY_ACCESS_USER = "SELECT AU.UUID, AU.USERNAME, AU.FIRST_NAME || ' ' || AU.LAST_NAME AS FULLNAME, AU.STATUS, COUNT(QI.ID) " +
        "FROM APP_USER AU " +
        "INNER JOIN PROFILE P ON P.ID = AU.PROFILE_ID " +
        "LEFT JOIN QUERY_INQUIRY QI ON QI.CREATED_BY_ID = AU.ID " +
        "WHERE P.PROFILE_NAME IN ('DB_PROFILE', 'SUPER_ADMIN_PROFILE') " +
        "GROUP BY AU.ID";
    ...
    public static String FETCH_LINK_ENVIRONMENT_VARIABLE_WITH_USER = "SELECT DISTINCT AU.ID, AU.EMAIL, AU.USERNAME, AU.FIRST_NAME || ' ' || AU.LAST_NAME AS FULL_NAME, " +
        "AU.IMG AS PROFILE_IMG, PRO.ID AS PROFILE_ID, PRO.PROFILE_NAME, PRO.DESCRIPTION, " +
        "AUE.ID AS LINK_ID, AUE.DATE_CREATED AS LINK_DATA, CASE WHEN AUE.DATE_CREATED IS NULL THEN FALSE ELSE TRUE END LINKED, AUE.ENV_VALUE, " +
        ...
        "WHERE AU.STATUS != %d AND AU.DATE_CREATED between '%s' and '%s' " +
        "ORDER by AU.ID DESC";
    ...
    public static String PROFILE_SETTING_STATISTICS = "SELECT CASE WHEN ROL.STATUS = 0 THEN 'INACTIVE-ROLE' WHEN ROL.STATUS = 1 THEN 'ACTIVE-ROLE' END AS NAME, COUNT(*) AS VALUE " +
        ...
        "UNION " +
        "SELECT CASE WHEN AU.STATUS = 0 AND AU.ORG_ACCOUNT = FALSE THEN 'INACTIVE-USER' WHEN AU.STATUS = 1 AND AU.ORG_ACCOUNT = FALSE THEN 'ACTIVE-USER' " +
         "WHEN AU.STATUS = 0 AND AU.ORG_ACCOUNT = TRUE THEN 'INACTIVE-ORG' WHEN AU.STATUS = 1 AND AU.ORG_ACCOUNT = TRUE THEN 'ACTIVE-ORG' END AS NAME, " +
         "COUNT(*) AS VALUE " +
        "FROM APP_USER AU WHERE AU.CREATED_BY_ID = %1$d AND AU.STATUS != 2 GROUP BY AU.STATUS, AU.ORG_ACCOUNT";
