...
public class QueryService {
    ...
    public static String DELETE_APP_USER_WEBHOOK_BY_WEBHOOK_ID_AND_APP_USER_ID = "DELETE FROM APP_USER_HOOKS AUH WHERE WEBHOOK_ID = %d AND APP_USER_ID = %d  ";
    ...
    public static String FETCH_LINK_ENVIRONMENT_VARIABLE_WITH_USER = "SELECT DISTINCT AU.ID, AU.EMAIL, AU.USERNAME, AU.FIRST_NAME || ' ' || AU.LAST_NAME AS FULL_NAME, " +
        ...
        "WHERE AU.STATUS != %d " +
        "ORDER by AU.ID DESC";
    public static String FETCH_LINK_WEB_HOOK_WITH_USER = "SELECT DISTINCT AU.ID, AU.EMAIL, AU.USERNAME, AU.FIRST_NAME || ' ' || AU.LAST_NAME AS FULL_NAME, " +
        "AU.IMG AS PROFILE_IMG, PRO.ID AS PROFILE_ID, PRO.PROFILE_NAME, AUH.DATE_CREATED AS LINK_DATA, " +
        "CASE WHEN AUH.DATE_CREATED IS NULL THEN FALSE ELSE TRUE END LINKED, AUH.ACCESS_TOKEN,  AUH.EXPIRE_TIME,  AUH.TOKEN_ID, " +
        "CASE WHEN AUH.STATUS IS NOT NULL THEN AUH.STATUS WHEN AU.STATUS = 0 OR AUH.STATUS = 0 THEN 0 ELSE 1 END AS LINK_STATUS " +
        "FROM APP_USER AU " +
        "INNER JOIN PROFILE PRO ON PRO.ID = AU.PROFILE_ID " +
        "LEFT JOIN APP_USER_HOOKS AUH ON AUH.APP_USER_ID = AU.ID AND AUH.WEBHOOK_ID = %d " +
        "WHERE AU.STATUS != %d " +
        "ORDER BY AU.ID DESC";
    ...
    public static String SERVICE_SETTING_STATISTICS = "SELECT CASE WHEN CRD.STATUS = 0 THEN 'INACTIVE-CREDENTIAL' WHEN CRD.STATUS = 1 THEN 'ACTIVE-CREDENTIAL' END AS NAME, COUNT(*) AS VALUE " +
        "FROM CREDENTIAL CRD WHERE CRD.CREATED_BY_ID = %1$d AND CRD.STATUS != 2 " +
        "GROUP BY CRD.STATUS " +
        "UNION \n" +
